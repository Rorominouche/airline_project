...
# =====================================================================
# CONFIGURATION TURSO CLOUD (Base Faustine & Ambrine)
# =====================================================================
...
if menu == "🔍 Search & Book Flights":
    ...
    try:
        # Table 'Route' au singulier
        routes = execute_query("SELECT departure_airport, arrival_airport FROM Route", fetch=True)
    ...
    if not departures or not arrivals:
        ...
        if st.button("Search Flights"):
            ...
            # Table 'Flight', pas de flight_id, colonnes price_economy, seats_economy, seats_business
            query = """
                SELECT flight_number, departure_datetime, price_economy, seats_economy, seats_business 
                FROM Flight 
        ...
        if st.session_state.search_results:
            ...
            for fl in st.session_state.search_results:
                fl_num, fl_time, price, seats_eco, seats_bus = fl
                ...
                with st.container():
                    ...
                    with col_book:
                        passenger_name = st.text_input("Passenger Full Name", key=f"name_{fl_num}")
                        seat_class = st.selectbox("Class", ["Economy", "Business"], key=f"class_{fl_num}")
                        
                        if st.button(f"Confirm Booking for {fl_num}", key=f"btn_{fl_num}"):
                            if not passenger_name:
                                ...
                                if (seat_class == "Economy" and seats_eco <= 0) or (seat_class == "Business" and seats_bus <= 0):
                                    ...
                                    try:
                                        # Table BOOKINGS / Bookings (Utilisation de fl_num à la place de fl_id)
                                        execute_query(
                                            "INSERT INTO Bookings VALUES (?, ?, ?, ?, ?)",
                                            (pnr, passenger_name, fl_num, seat_class, current_date)
                                        )
                                        
                                        if seat_class == "Economy":
                                            execute_query("UPDATE Flight SET seats_economy = seats_economy - 1 WHERE flight_number = ?", (fl_num,))
                                        else:
                                            execute_query("UPDATE Flight SET seats_business = seats_business - 1 WHERE flight_number = ?", (fl_num,))
...
# =====================================================================
    ...
    if st.button("Generate Boarding Pass"):
        if len(pnr_input) != 6:
            ...
            # Join adapté avec Flight au singulier et sur flight_number
            query = """
                SELECT b.passenger_name, b.seat_class, f.flight_number, f.departure_datetime, f.route_id, f.tail_number
                FROM Bookings b
                JOIN Flight f ON b.flight_id = f.flight_number
            ...
            try:
                ...
                st.error(f"Erreur de recherche PNR : {e}")
...
# =====================================================================
    ...
    # Requête recalibrée sur la table Flight au singulier et flight_number
    query = """
...
# =====================================================================
# MODULE 4 : FLIGHT OCCUPANCY (Yield & Analytics)
# =====================================================================
    ...
    # Tables Flight et Fleet au singulier. Colonnes de stock réalignées (seats_economy, seats_business)
    query = """
        SELECT f.flight_number, f.route_id, f.departure_datetime, 
               f.seats_economy, ac.economy_capacity,
               f.seats_business, ac.business_capacity
        FROM Flight f
    ...
    try:
        ...
        if not flights_stock:
            ...
            for flight in flights_stock:
                ...
                # Calcul propre : Vendu = Capacité de la Flotte - Reste disponible dans le Vol
